...
export default async function AdxRechargesPage() {
  const supabase = await createClient();

  const { data } = await supabase
    .from("token_requests")
    .select("*, atelier:ateliers(nom)")
  ...
  const requests = (data ?? []).map((r: Record<string, unknown>) => ({
    ...
    code_indice:  undefined,
  }));

  async function generateCode(requestId: string, tokens: number, jours: number)
    ...
    const { data: code, error } = await sb.rpc("generer_code_token", {
      p_request: requestId, p_tokens: tokens, p_jours: jours,
    });
    if (error) return { ok: false, message: error.message };
    revalidatePath("/adx/recharges");
    return { ok: true, code: code as string };
  }

  async function markShipped(requestId: string, transporteur: string, numero: string)
    : Promise<{ ok: boolean; message?: string }> {
    "use server";
    const sb = await createClient().catch(() => null);
    if (!sb) return { ok: false, message: "Erreur serveur." };
    const { error } = await sb.from("token_requests")
  ...
  return (
    <>
      ...
      <RechargeTable requests={requests} generateCodeAction={generateCode} markShippedAction={markShipped} />
    </>
  );
}
